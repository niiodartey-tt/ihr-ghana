...
function SectionLabel({ text, light = false }: { text: string; light?: boolean }) {
  return (
    <p style={{
      fontFamily: "var(--font-cormorant)", fontSize: "0.7rem",
...
const VALUES = [
  { name: "Adinkrahene", value: "Leadership", desc: "We believe genuine leadership is not a title bestowed but a gravity earned — through integrity, consistency, and the courage to make difficult decisions in service of those who follow.", color: "#C8651A" },
  { name: "Nea Onnim", value: "Continuous Learning", desc: "Every engagement is an opportunity to know more, understand deeper, and serve better. We hold ourselves to a standard of perpetual growth — because the organisations we serve deserve advisors who never stop learning.", color: "#3A7D6E" },
  { name: "Agyindawuru", value: "Alertness", desc: "We remain fully present — to the shifts in your industry, the undercurrents in your culture, the unspoken needs of your people. Alertness is not anxiety; it is the discipline of paying attention before it becomes necessary.", color: "#A83E10" },
  { name: "Akoma", value: "Candour & Care", desc: "We tell you what you need to hear, not just what you want to hear — delivered with genuine care for your success. Candour without compassion is cruelty; compassion without candour is cowardice. We choose both.", color: "#2D6459" },
];

const APPROACH = [
  { number: "01", title: "Discover", desc: "We begin by listening — deeply, without assumption. Every organisation has a unique context, history, and set of pressures. Before we offer a single recommendation, we immerse ourselves in yours." },
  { number: "02", title: "Diagnose", desc: "We identify root causes, not just symptoms. Using structured frameworks and candid conversations, we map the landscape of your people challenges with precision and honesty." },
  { number: "03", title: "Design", desc: "We co-create solutions tailored to your organisation — not templates lifted from a handbook. Every strategy, system, and intervention is built with your context, your culture, and your people in mind." },
  { number: "04", title: "Deliver & Sustain", desc: "We implement with rigour and build internal capability so the change endures beyond our engagement. Our measure of success is not a delivered report — it is a transformed organisation." },
];
      ...
      <div style={{ position: "relative" }}>
          ...
          <div style={{ maxWidth: "1100px", margin: "0 auto", width: "100%", position: "relative" }}>
            ...
            <Reveal>
              <div style={{ borderLeft: "3px solid #C8651A", paddingLeft: "2rem", marginBottom: "5rem" }}>
                <p style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.6rem, 3vw, 2.6rem)", fontWeight: 600, color: "#1A1410", lineHeight: 1.25, fontStyle: "italic", maxWidth: "780px" }}>
                  "We exist because people are not resources to be managed — they are the very reason organisations exist at all."
                </p>
              </div>
            </Reveal>
        ...
        <section style={{
          ...
          <div style={{ maxWidth: "1100px", margin: "0 auto", width: "100%", position: "relative" }}>
            <Reveal>
              ...
              <h2 style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(2.2rem, 4vw, 3.8rem)", fontWeight: 700, color: "#F5EDD8", lineHeight: 1.05, marginBottom: "4rem" }}>
                Our Values
              </h2>
            </Reveal>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "2px" }}>
              {VALUES.map((v, i) => (
                <Reveal key={v.value} delay={i * 0.1}>
                  <motion.div
                    style={{ padding: "3rem", borderTop: `2px solid ${v.color}`, background: "rgba(255,255,255,0.03)", height: "100%" }}
                    whileHover={{ background: "rgba(255,255,255,0.07)" }}
                    transition={{ duration: 0.4, ease: EASE }}
                  >
                    <p style={{ fontFamily: "var(--font-cormorant)", fontSize: "0.65rem", letterSpacing: "0.25em", textTransform: "uppercase", color: v.color, marginBottom: "0.5rem", fontWeight: 500 }}>{v.name}</p>
                    <h3 style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.5rem, 2.5vw, 2.2rem)", fontWeight: 700, color: "#F5EDD8", lineHeight: 1.05, marginBottom: "1rem" }}>{v.value}</h3>
                    <p style={{ fontSize: "0.9rem", lineHeight: 1.8, color: "rgba(245,237,216,0.62)", fontWeight: 300 }}>{v.desc}</p>
                  </motion.div>
                </Reveal>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6rem", alignItems: "start" }}>
              <div>
                <Reveal direction="left">
                  ...
                  <h2 style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(2.5rem, 5vw, 4.5rem)", fontWeight: 700, color: "#F5EDD8", lineHeight: 1.0, marginBottom: "1.5rem" }}>
                    Our Proven<br />Approach
                  </h2>
                  <p style={{ fontSize: "0.95rem", lineHeight: 1.85, color: "rgba(245,237,216,0.75)", fontWeight: 300 }}>
              ...
              <ul style={{ display: "flex", flexDirection: "column" }}>
                {APPROACH.map((step, i) => (
                  <Reveal key={step.number} delay={i * 0.1}>
                    <motion.li
                      style={{ display: "flex", gap: "1.5rem", alignItems: "flex-start", padding: "1.5rem 0", borderBottom: "1px solid rgba(245,237,216,0.2)", listStyle: "none" }}
                      whileHover={{ x: 6 }}
                      transition={{ duration: 0.3, ease: EASE }}
                    >
                      <span style={{ fontFamily: "var(--font-cormorant)", fontSize: "2rem", fontWeight: 300, color: "rgba(245,237,216,0.3)", lineHeight: 1.2, minWidth: "3rem" }}>{step.number}</span>
                      <div>
                        <p style={{ fontSize: "1rem", fontWeight: 500, color: "#F5EDD8", marginBottom: "0.4rem", letterSpacing: "0.02em" }}>{step.title}</p>
                        <p style={{ fontSize: "0.88rem", lineHeight: 1.75, color: "rgba(245,237,216,0.7)", fontWeight: 300 }}>{step.desc}</p>
                      </div>
                    </motion.li>
                  </Reveal>
                ))}
        ...
        <section style={{
          ...
          <div style={{ maxWidth: "1100px", margin: "0 auto", width: "100%", position: "relative" }}>
            ...
            <Reveal delay={0.3}>
              <div style={{ marginTop: "5rem", textAlign: "center", paddingTop: "3rem", borderTop: "1px solid rgba(26,20,16,0.1)" }}>
                <p style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.5rem, 3vw, 2.5rem)", fontWeight: 700, color: "#1A1410", marginBottom: "1rem", letterSpacing: "-0.01em" }}>
                  Ready to transform your people strategy?
                </p>
                <p style={{ fontSize: "0.95rem", color: "rgba(26,20,16,0.55)", lineHeight: 1.7, maxWidth: "480px", margin: "0 auto 2rem" }}>
                  Let's start with a conversation. No obligation — just clarity on what's possible for your organisation.
                </p>
                <motion.a
                  href="/contact"
                  style={{ display: "inline-block", background: "#1A1410", color: "#F5EDD8", padding: "1rem 2.8rem", fontSize: "0.75rem", letterSpacing: "0.2em", textTransform: "uppercase", textDecoration: "none", fontWeight: 400 }}
